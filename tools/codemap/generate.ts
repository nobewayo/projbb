...
async function main() {
  ...
  const files = await globby(include, { ignore: exclude, gitignore: true });
  ...
  fs.writeFileSync(
    'codemap.json',
    JSON.stringify({ generatedAt: new Date().toISOString(), count: entries.length, files: entries }, null, 2),
  );
