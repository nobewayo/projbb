...
detect_base() {
  ...
  if git rev-parse --abbrev-ref --symbolic-full-name @{u} >/dev/null 2>&1; then
    git rev-parse --abbrev-ref --symbolic-full-name @{u}; return
  fi
  if git rev-parse origin/HEAD >/dev/null 2>&1; then
    git symbolic-ref -q --short origin/HEAD || echo "origin/main"; return
  fi
...
# If BASE is a remote ref, try to fetch quietly; ignore failures (offline safe)
if [[ "$BASE" == origin/* ]]; then git fetch -q origin >/dev/null 2>&1 || true; fi
