...
- a Vite + React client that now renders the deterministic top-right anchored grid with live hit-testing across the fixed 10-row field, keeps the right panel and bottom dock chrome locked to the stage footprint, surfaces the blocking reconnect overlay mandated by the spec, draws the development room background + tile overlays, and layers placeholder avatar PNGs with eased movement interpolation, grid visibility toggles, hidden-grid hover outlines, and underfoot username labels while awaiting authoritative `move:ok` / `move:err` replies
...
- The grid canvas now draws the development room background, sorts occupants by tile depth, renders placeholder avatar PNGs with eased interpolation, keeps usernames anchored beneath each avatar's feet, and exposes quick admin toggles for grid visibility and hidden-grid hover outlines.
