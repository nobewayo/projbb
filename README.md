...
- a Vite + React client that renders the deterministic top-right anchored grid, keeps the blocking reconnect overlay mandated by the spec, streams authoritative chat history with a freeform composer that listens globally (type anywhere on the stage, press **Enter** to send, **Esc** to cancel), paints dev room items beneath avatars with click-through hit testing so the right panel can surface spec-compliant pickup gating copy, exposes spec-compliant right-click context menus for tiles, items, and avatars, and now hydrates persisted mute/report state, prunes muted occupants from the retained chat transcript, consumes Redis-backed social broadcasts, reacts to realtime trade lifecycle updates (invite/pending/accept/decline/cancel/complete) without redundant REST polls, replays the latest trade banner after reconnect, and composes the panel UI from extracted `ProfilePanel` / `InventoryCard` components driven by a reusable toast hook while keeping the visible chat window at an authoritative 200-message ceiling. Context menus now co-display occupant actions and tile inventory even when avatars overlap dropped items, the hit test follows the sprite bounds (so right-clicking an oversized couch still selects it), the overlay footprint has been reduced by roughly 75% to match the latest UX feedback, avatar menus hide the mute/report affordances when you right-click yourself so the action list mirrors production moderation rules, and self-targeted trade actions are hard-blocked in the action handler so you can never initiate a trade with your own avatar. Profile fetches route through a hardened HTTP helper that trims and parses raw payload text before schema validation so the “Malformed response payload” error is eliminated even when proxies inject stray whitespace.
...
1. Persist and surface historical trade summaries (who traded, when, and what changed hands) so profiles/backpack history can expose authoritative trade logs.
2. Layer in time-based chat retention/archival (beyond the live 200-message ring) so muted/aged transcripts can be exported, audited, or aged out without bloating the primary table.
3. Extend the realtime/client test suite to cover negotiation proposal updates, readiness toggles, and the hardened REST helper behaviour (unit + integration coverage).
